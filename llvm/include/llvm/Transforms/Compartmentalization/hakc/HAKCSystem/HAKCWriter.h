...
public:
  ...
  ~HAKCWriter() { errs() << "DESTROYING HAKC WRITER\n"; }
  ...
  bool verifyFunction(Function *F) {
    return llvm::verifyFunction(*F, os.get());
  }
...
public:
  explicit HAKCLogger(HAKCLogLevel log_level)
      : HAKCStreams(), TempLogLevel(Verbose) {
  ...
  void SetAllWriterConfiguredLogLevels(HAKCLogLevel log_level) {
    for (auto &stream : HAKCStreams) {
      stream->SetConfiguredLogLevel(log_level);
    }
  }
  ...
  HAKCLogger &operator<<(raw_ostream &(*manip)(raw_ostream &)) {
    for (auto &stream : HAKCStreams) {
      if (TempLogLevel && (TempLogLevel >= stream->GetConfiguredLogLevel())) {
        stream->operator<<(manip);
      }
    }
    return *this;
  }
