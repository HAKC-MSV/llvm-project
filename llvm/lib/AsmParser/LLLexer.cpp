...
#define INSTKEYWORD(STR, Enum)                                                 \
  ...
  INSTKEYWORD(indirectbr, IndirectBr); INSTKEYWORD(invoke, Invoke);
      INSTKEYWORD(resume, Resume); INSTKEYWORD(unreachable, Unreachable);
      INSTKEYWORD(callbr, CallBr);

      INSTKEYWORD(alloca, Alloca); INSTKEYWORD(load, Load);
      INSTKEYWORD(store, Store); INSTKEYWORD(cmpxchg, AtomicCmpXchg);
      INSTKEYWORD(atomicrmw, AtomicRMW); INSTKEYWORD(fence, Fence);
      ...
      INSTKEYWORD(landingpad, LandingPad); INSTKEYWORD(cleanupret, CleanupRet);
      INSTKEYWORD(catchret, CatchRet); INSTKEYWORD(catchswitch, CatchSwitch);
      INSTKEYWORD(catchpad, CatchPad); INSTKEYWORD(cleanuppad, CleanupPad);
...
#define DWKEYWORD(TYPE, TOKEN)                                                 \
  do {                                                                         \
    if (Keyword.starts_with("DW_" #TYPE "_")) {                                \
      ...
      DWKEYWORD(TAG, DwarfTag); DWKEYWORD(ATE, DwarfAttEncoding);
      DWKEYWORD(VIRTUALITY, DwarfVirtuality); DWKEYWORD(LANG, DwarfLang);
      DWKEYWORD(CC, DwarfCC); DWKEYWORD(OP, DwarfOp);
...
#define DBGRECORDTYPEKEYWORD(STR)                                              \
  do {                                                                         \
    if (Keyword == "dbg_" #STR) {                                              \
      ...
      DBGRECORDTYPEKEYWORD(value); DBGRECORDTYPEKEYWORD(declare);
      DBGRECORDTYPEKEYWORD(assign); DBGRECORDTYPEKEYWORD(label);
#undef DBGRECORDTYPEKEYWORD
      ...
      // by the CFE to avoid forcing it to deal with 64-bit numbers.
