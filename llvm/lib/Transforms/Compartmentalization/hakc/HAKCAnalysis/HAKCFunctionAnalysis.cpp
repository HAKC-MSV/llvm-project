...
void HAKCFunctionAnalysis::UpdateHAKCFunctionParameters() const {
  ...
  CommonHAKCAnalysis::CommonHAKCAnalysis::getWriter()
...
bool HAKCFunctionAnalysis::userInFunction(Value *User) const {
  ...
  CommonHAKCAnalysis::CommonHAKCAnalysis::getWriter(Error)
      << "Unexpected user: " << User << "\n";
  throw std::exception();
}
...
Instruction *HAKCFunctionAnalysis::getUserInst(User *user) {
  if (auto *inst = dyn_cast<Instruction>(user)) {
    ...
    CommonHAKCAnalysis::getWriter(Error) << "Unexpected user: " << user << "\n";
    throw std::exception();
  }
}
...
void HAKCFunctionAnalysis::TemporalAnalysis() {
  ...
  for (auto ptr : PointerManager.ManagedPointers()) {
    ...
    ptr->GetAllUses(Uses);
    CommonHAKCAnalysis::getWriter(Error) << *ptr << "\n";
    for (auto Use : Uses) {
      CommonHAKCAnalysis::getWriter(Error) << "\tAnalyzing Use" << *Use << "\n";
      auto *I = Use->getUser();
      if (auto *Call = dyn_cast<CallInst>(I)) {
...
void HAKCFunctionAnalysis::TemporalAnalysisHandleCall(
    ManagedHAKCPointerUseP CallUse) {
...
void HAKCFunctionAnalysis::TemporalAnalysisHandleStore(
    ManagedHAKCPointerUseP StoreUse) {
...
void HAKCFunctionAnalysis::AddInstrumentation(bool RelocateSection) {
  ...
  if (!SetupHasRun) {
    CommonHAKCAnalysis::getWriter() << __FUNCTION__ << " calling setup for "
                                    << GetFunction().getName() << "\n";
    setup();
  }
  CommonHAKCAnalysis::getWriter()
...
void HAKCFunctionAnalysis::ReplaceInstructionOperand(Instruction *I,
  ...
  if (auto *Oper = dyn_cast<BitCastOperator>(V)) {
    ...
    CommonHAKCAnalysis::getWriter(Error) << "Could not find ";
    if (auto *F = dyn_cast<Function>(OldValue)) {
      CommonHAKCAnalysis::getWriter(Error) << F->getName();
    } else {
      CommonHAKCAnalysis::getWriter(Error) << OldValue << "\n";
    }
    CommonHAKCAnalysis::getWriter(Error) << " in " << *I << "\n";
    throw std::exception();
  }
