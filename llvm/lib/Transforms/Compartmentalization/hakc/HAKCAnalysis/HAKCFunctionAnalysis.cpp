...
Instruction *HAKCFunctionAnalysis::addCompartmentTransferCall(
  ...
  HAKCFunctionAnalysis::getLogger(Verbose) << "Created transfer for ";
  if (!IsData) {
    HAKCFunctionAnalysis::getLogger(Verbose) << Operand->getName();
  } else {
    HAKCFunctionAnalysis::getLogger(Verbose) << Operand;
  }
  HAKCFunctionAnalysis::getLogger(Verbose) << ": " << TransferCall << "\n";
...
void HAKCFunctionAnalysis::handleComparison(CmpInst *compare) {
  ...
  if (isCompartmentalizedFunction()) {
    ...
    if (DebugActive) {
      if (arg0NeedsAuth) {
        HAKCFunctionAnalysis::getLogger(Verbose) << "Argument 0 needs auth\n";
      } else {
        HAKCFunctionAnalysis::getLogger(Verbose)
            << "Argument 0 does not need auth\n";
      }
      if (arg1NeedsAuth) {
        HAKCFunctionAnalysis::getLogger(Verbose) << "Argument 1 needs auth\n";
      } else {
        HAKCFunctionAnalysis::getLogger(Verbose)
    ...
    if (arg0NeedsAuth && arg1NeedsAuth) {
      ...
      if (arg0NeedsAuth) {
        HAKCFunctionAnalysis::getLogger(Verbose) << "Registering argument 0\n";
        AddManagedPointer(compare->getOperandUse(0));
      } else {
        HAKCFunctionAnalysis::getLogger(Verbose)
      ...
      if (arg1NeedsAuth) {
        HAKCFunctionAnalysis::getLogger(Verbose) << "Registering argument 1\n";
        AddManagedPointer(compare->getOperandUse(1));
      }
    }
  } else {
...
void HAKCFunctionAnalysis::handleBinaryOperator(BinaryOperator *binOp) {
  ...
  if (argNeedsAuthentication(binOp->getOperandUse(0)) &&
    ...
    HAKCFunctionAnalysis::getLogger(Verbose) << "Registering both operands\n";
...
void HAKCFunctionAnalysis::handleCall(CallInst *call) {
  ...
  HAKCFunctionAnalysis::getLogger(Verbose) << "Handling call " << *call << "\n";
...
void HAKCFunctionAnalysis::AddInstrumentation(bool RelocateSection) {
  ...
  for (auto &HAKCPointer : PointerManager.ManagedPointers()) {
    HAKCFunctionAnalysis::getLogger(Verbose) << *HAKCPointer << "\n+++\n";
  ...
  if (modifiedFunction()) {
    ...
    HAKCFunctionAnalysis::getLogger(Verbose) << GetFunction() << "\n";
...
void HAKCFunctionAnalysis::ReplaceInstructionOperand(Instruction *I,
  ...
  if (auto *Oper = dyn_cast<BitCastOperator>(V)) {
    ...
    HAKCFunctionAnalysis::getLogger(Verbose) << " in " << *I << "\n";
    throw std::exception();
  }
