...
HAKCFunctionAnalysis::HAKCFunctionAnalysis(Function *F, HAKCModuleAnalysis& ModuleAnalysis)
  : ModuleAnalysis(ModuleAnalysis), _PointerManager(F, ModuleAnalysis),
    ...
    void HAKCFunctionAnalysis::setup() {
      ...
      getLogger(Debug) << "Function Analysis setup has run for " << CurrentFunction->getName() << "\n";
    }
 ...
 * @return
 */
BasicBlock *HAKCFunctionEnforcement::findDominatorUseBlock(Value *Ptr, const std::set<Instruction *> &Users) const {
  ...
  if (auto *I = dyn_cast<Instruction>(Ptr)) {
    if (!isa<AllocaInst>(Ptr)) {
      ...
      for (auto *BB : BasicBlocks) {
        if (!Dominator) {
          ...
          Dominator = DTree.findNearestCommonDominator(
                  Dominator->getFirstNonPHIOrDbgOrLifetime(),
                  BB->getFirstNonPHIOrDbgOrLifetime())->getParent();
        }
      }
      if (!Dominator) {
    ...
    bool HAKCFunctionAnalysis::argNeedsAuthentication(Use &arg) {
        if (auto *call = dyn_cast<CallInst>(arg.getUser())) {
            if (auto *inlineAsm = dyn_cast<InlineAsm>(call->getCalledOperand())) {
                ...
                return ((arg->getType()->isPointerTy() || isa<PtrToIntInst>(arg.get()))) &&
                      (ModuleAnalysis.GetCommonAnalysis().IsSafeTransitionFunction(
                            call->getCalledFunction()) ||
                      PointerManager.IsIntrinsicNeedingAuthentication(call));
        }
      }
      return (!isa<Function>(arg) && PointerManager.PointerIsEligibleForManagement(arg));
    }
    ...
    bool HAKCFunctionAnalysis::phiNodeUsesValue(PHINode *PhiNode, Value *target,
      ...
      for (auto &Val : PhiNode->incoming_values()) {
        ...
        if (Val.get() == target || def == target) { return true; }
    ...
    void HAKCFunctionAnalysis::HandleInstruction(Instruction *I) {
      ...
      if (auto *call = dyn_cast<CallInst>(I)) { handleCall(call);
      } else if (auto *load = dyn_cast<LoadInst>(I)) { handleLoad(load);
      } else if (auto *store = dyn_cast<StoreInst>(I)) { handleStore(store);
      } else if (auto *binOp = dyn_cast<BinaryOperator>(I)) { handleBinaryOperator(binOp); }
    }

    void HAKCFunctionEnforcement::HandleInstruction(Instruction *I) {
      if (dyn_cast<CallInst>(I) || dyn_cast<LoadInst>(I) || dyn_cast<StoreInst>(I) || dyn_cast<BinaryOperator>(I)) {
    ...
    Instruction *HAKCFunctionAnalysis::getUserInst(User *user) {
        if (auto *inst = dyn_cast<Instruction>(user)) { return inst; }
    ...
    Value *HAKCFunctionAnalysis::getDef(Value *V, bool followLoad) const {
        ...
        if (!def) {
            HAKCFunctionAnalysis::getLogger(Fatal)<< "Could not find definition for " << V << "\n";
            throw std::exception();
        }
        return def;
    }
    ...
    void HAKCFunctionAnalysis::handleStore(StoreInst *Store) {
      ...
      void HAKCFunctionAnalysis::handleBinaryOperator(BinaryOperator *binOp) {
         ...
         * ptr | 0xFFFF
         */
        if (argNeedsAuthentication(binOp->getOperandUse(0)) && argNeedsAuthentication(binOp->getOperandUse(1))) {
    ...
    void HAKCFunctionEnforcement::handleComparison(CmpInst *compare) {
        GetLogger(Verbose, !DebugActive) << "Checking comparison " << *compare << "\n";

        MaybeAddCompareToDirectUsers(compare);

        if (isa<ConstantPointerNull>(compare->getOperand(0)) || isa<ConstantPointerNull>(compare->getOperand(1))) {
          GetLogger(Verbose, !DebugActive) << "\tComparisons with null do not need authentication\n";
          return;
        }
          if (isa<Operator>(compare->getOperand(0)) || isa<Operator>(compare->getOperand(1))) {
            bool comparisonIsWithConstant = false;
            if (const auto *bitCastOperator0 = dyn_cast<Operator>(compare->getOperand(0))) {
              if (const auto *ci = dyn_cast<ConstantInt>(bitCastOperator0->getOperand(0))) {
                comparisonIsWithConstant = (ci->getZExtValue() < user_space_end);
              }
            }
            if (!comparisonIsWithConstant) {
              if (const auto *bitCastOperator1 = dyn_cast<Operator>(compare->getOperand(1))) {
                if (const auto *ci = dyn_cast<ConstantInt>(bitCastOperator1->getOperand(0))) {
                  comparisonIsWithConstant = (ci->getZExtValue() < user_space_end);
                }
              }
            }
            if (comparisonIsWithConstant) {
              GetLogger(Verbose, !DebugActive) << "\tComparisons with constant integers do not need authentications\n";
              return;
            }
        }

        if (isCompartmentalizedFunction()) {
            const bool arg0NeedsAuth = argNeedsAuthentication(compare->getOperandUse(0)) && !isa<GlobalValue>(getDef(compare->getOperand(0), false));
            const bool arg1NeedsAuth = argNeedsAuthentication(compare->getOperandUse(1)) && !isa<GlobalValue>(getDef(compare->getOperand(1), false));

            GetLogger(Verbose, !DebugActive) << "Argument 0 " << (arg0NeedsAuth ? "needs auth" : "does not need auth") << "\n";
            GetLogger(Verbose, !DebugActive) << "Argument 1 " << (arg1NeedsAuth ? "needs auth" : "does not need auth") << "\n";
            ...
            if (arg0NeedsAuth && arg1NeedsAuth) {
                ...
                if (arg0NeedsAuth) {
                    ...
                    GetLogger(Verbose, !DebugActive) << "Argument 1 (" << compare->getOperand(1) << " ) already authenticated\n";
                }
                if (arg1NeedsAuth) {
    ...
    bool HAKCFunctionAnalysis::globalShouldBeTransferred(
        ...
        if (auto *globalValue =
            ...
            if (globalValue->getValueType()->isArrayTy() && globalValue->getValueType()->getArrayElementType()->isIntegerTy(8)) {
                return false;
            }

            if (const auto *call = dyn_cast<CallInst>(globalValueArg.getUser())) {
              if (!ModuleAnalysis.GetCommonAnalysis().FunctionIsAnalysisCandidate(call->getCalledFunction())) { return false; }
                return true;
            }
    ...
    void HAKCFunctionAnalysis::handleCall(CallInst *call) {
        ...
        if (ModuleAnalysis.GetCommonAnalysis().IsHAKCFunction(call->getCalledFunction())) {
        ...
        bool needsAuthenticatedArgs = call->isInlineAsm() || (ModuleAnalysis.FunctionIsInAnalysisSet(call->getCalledFunction()) &&
          !CommonHAKCAnalysis::IsOutsideTransferFunc(call->getCalledFunction()));

        if (isa<IntrinsicInst>(call)) {
          needsAuthenticatedArgs = PointerManager.IsIntrinsicNeedingAuthentication(call);
        }

        GetLogger(Verbose, !DebugActive) << *call << (needsAuthenticatedArgs ? " needs authenticated args" : " does not need authenticated args") << "\n";
        ...
        if (call->isIndirectCall()) {
          ...
          for (auto &arg : call->args()) {
            ...
            GetLogger(Verbose, !DebugActive) << "Argument " << *arg << " for " << *call << " does not need authentication\n";
          }
            ...
            for (auto &arg: call->args()) {
                ...
                if (auto *glob = dyn_cast<GlobalValue>(def)) {
                    if (globalShouldBeTransferred(arg)) {
                        GetLogger(Verbose, !DebugActive) << "Global " << glob->getName() << " used by " << *call << "\n";
                        GlobalArgumentUses[glob].insert(call);
                        AddManagedPointer(arg);
                }
                GetLogger(Verbose, !DebugActive) << "Global " << glob->getName() << " should not be transferred to " << *call << "\n";
                    ...
                    for (auto &val : phiNode->incoming_values()) {
                        ...
                        if (auto *globVal = dyn_cast<GlobalValue>(valDef)) {
                            if (globalShouldBeTransferred(val)) {
                                GetLogger(Verbose, !DebugActive) << "Global " << globVal->getName() << " used by " << *call << "\n";
                                GlobalArgumentUses[globVal].insert(call);
                            }
                            GetLogger(Verbose, !DebugActive) << "Global " << globVal->getName() << " should not be transferred to " << *call << "\n";
                        }
                    }
                } else if (isa<AllocaInst>(def)) {
                    if (!ModuleAnalysis.GetCommonAnalysis().FunctionIsAnalysisCandidate(call->getCalledFunction())) {
                        GetLogger(Verbose, !DebugActive) << "Function called by " << *call << " is not an analysis candidate\n";
                    }
                }
            }
    ...
    void HAKCFunctionAnalysis::TypeUseAnalysis() {
        GetLogger(Debug, !DebugActive) << "!!!! Starting Function Temporal Analysis !!!!\n";
        ...
        for (auto ptr : PointerManager.ManagedPointers()) {
            ...
            for (auto Use : Uses) {
                ...
                if (dyn_cast<CallInst>(I)) {
                    ...
                    GetLogger(Debug, !DebugActive) << "\t\tSkipping Use because it is not a call, load, or store!\n";
                }
            }
        }
        GetLogger(Debug, !DebugActive) << "!!!! Ending Function Temporal Analysis !!!!\n";
    }

    void HAKCFunctionAnalysis::AddPermissionUse(const ManagedHAKCPointer &ManagedPointer, const TypePerms perm) const {
        // TODO: fix segfault on line below when GetType() is null?
        const auto HAKCTy = ManagedPointer.GetType();
        if (!HAKCTy) {
          // HAKCFunctionAnalysis::getLogger(Fatal) << "ManagedPointer.GetType() is Null for " << ManagedPointer << "!\n";
          // throw std::exception();
          return;
        }
    ...
    void HAKCFunctionAnalysis::TypeUseHandleLoad(const ManagedHAKCPointerUseP &LoadUse) const {
      if (LoadUse->getOperandNo() == LoadInst::getPointerOperandIndex()) {
        AddPermissionUse(LoadUse->getManagedPtr(), Read);
      } else {
        getLogger(Debug)
            << "LoadUse was not operand 0! (Is this even possible?)" << *LoadUse
            << "\n";
      }
    }

    void HAKCFunctionAnalysis::TypeUseHandleStore(const ManagedHAKCPointerUseP &StoreUse) const {
      ...
      if (StoreUse->getOperandNo() == StoreInst::getPointerOperandIndex()) {
        ...
        getLogger(Debug)
            << "StoreUse was not operand 1! (Is this even possible?)" << *StoreUse
            << "\n";
      }
    }
    ...
    bool HAKCFunctionAnalysis::modifiedFunction() const {
        return !(PointerManager.empty() && GlobalArgumentUses.empty() && DirectFunctionCallSet.empty()
                && PointerManager.GetTotalAdditions() == 0 && CompartmentTransferCount == 0);
    }
    ...
    HAKCFunctionEnforcement::HAKCFunctionEnforcement(Function *F, HAKCModuleAnalysis& ModuleAnalysis, HAKCTransformer &Transformer, HAKCServerClientBase &Client)
                          : HAKCFunctionAnalysis(F, ModuleAnalysis), Transformer(Transformer), Client(Client), DTree(*F) {
    ...
    void HAKCFunctionEnforcement::UpdateHAKCFunctionParameters() const {
      ...
      GetLogger(Verbose, !DebugActive) << "Updating parameters for the following HAKC functions:\n";
      ...
      if (CommonHAKCAnalysis::IsOutsideTransferFunc(F)) {
        const auto TransferTargetName = F->getName().substr(OUTSIDE_TRANSFER_PREFIX.size());
        TransferTarget = F->getParent()->getFunction(TransferTargetName);
      }

      const auto TargetCompartment = Client.GetDivision(TransferTarget).GetHAKCCompartment();
      for (auto *CallI : HAKCFunctionCalls) {
        if (auto HAKCTransferFunction = ModuleAnalysis.GetCommonAnalysis().GetHAKCTransferDefinition(CallI->getCalledFunction())) {
          GetLogger(Verbose, !DebugActive) << "Updating HAKC call parameters for " << CallI << "\n";
          GetLogger(Verbose, !DebugActive) << "Updating index " << HAKCTransferFunction->GetCompartmentIdIdx() << " (" << CallI->getArgOperand(HAKCTransferFunction->GetCompartmentIdIdx()->getZExtValue())<< ") to " << TargetCompartment.GetCompartmentIDValue() << "\n";
          UpdateHAKCFunctionParameters(CallI, TargetCompartment, HAKCTransferFunction);
          GetLogger(Verbose, !DebugActive) << "After update call is " << CallI << "\n";
        } else {
          GetLogger(Verbose, !DebugActive) << "No HAKC Transfer function found for "<< CallI->getCalledFunction()->getName() << "\n";
        }
      }
    }


    void HAKCFunctionEnforcement::CheckCompareOperandForDirectFunctionUse(CmpInst *CmpI, const unsigned OpNo) {
        auto *Op = getDef(CmpI->getOperand(OpNo), false);
        if (auto *func = dyn_cast<Function>(Op)) {
            if (ModuleAnalysis.GetCommonAnalysis().ValueShouldBeReplacedWithTransfer(func, Client)) {
              GetLogger(Verbose, !DebugActive) << "Adding comparison to directFunctionUsers for argument " << std::to_string(OpNo) << "\n";
    ...
    std::string HAKCFunctionEnforcement::getHAKCFunctionSectionName() const {
      const auto Compartment = Client.GetDivision(CurrentFunction).GetHAKCCompartment();

      std::string sectionName = HAKC_SECTION_PREFIX.str() + std::to_string(Compartment.GetCompartmentIDValue());
      sectionName += (CurrentFunction->getSection().empty()) ? ".text" : CurrentFunction->getSection().str();
    ...
    bool HAKCFunctionEnforcement::isCompartmentalizedFunction() {
      return CommonHAKCAnalysis::IsCompartmentalizedFunction(CurrentFunction, Client);
    }
    ...
    void HAKCFunctionEnforcement::UpdateHAKCFunctionParameters(
      ...
      GetLogger(Verbose, !DebugActive)
          << "Setting "
          << *CallI->getArgOperand(HAKCTransferFunction->GetCompartmentIdIdx()->getZExtValue())
          << " to be " << *TargetCompartment.GetCompartmentID() << "\n";
      CallI->setOperand(HAKCTransferFunction->GetCompartmentIdIdx()->getZExtValue(), TargetCompartment.GetCompartmentID());

      if (HAKCTransferFunction->GetDivisionIdIdx() != nullptr) {
        auto *F = CallI->getFunction();
        HAKCCompartmentDivision Division = CommonHAKCAnalysis::IsOutsideTransferFunc(F) ? Client.GetDivision(CommonHAKCAnalysis::GetOriginalFunctionFromTransferFunction(F)) : Client.GetDivision(F);
    ...
    void HAKCFunctionEnforcement::CheckForValidCompartmentTransitionAndUpdateIntraCompartmentCalls() {
      ...
      for (auto *call : DirectFunctionCallSet) {
        ...
        if (Target->getZExtValue() ==
          ...
          if (call->getCalledFunction()->isVarArg()) {
            auto *VariadicTransfer = GetTransformer().CreateTransferToVariadic(call, &PointerManager);
    ...
    void HAKCFunctionEnforcement::CreateAuthenticatedPointersAndAllClones() {
      ...
      for (auto &ManagedPtr : SortedPointers) {
        ...
        if (ManagedPtr->GetAuthenticatedPointer()) {
          GetLogger(Verbose, !DebugActive) << "Authenticated Pointer for " << *ManagedPtr << ": " << ManagedPtr->GetAuthenticatedPointer() << "\n";
        }
      }
      for (auto &ManagedPtr : SortedPointers) {
        CreatePointerUseClones(ManagedPtr);
        GetLogger(Verbose, !DebugActive) << "Created Authenticated and Protected Copies for " << *ManagedPtr << "\n";
      }
    }
    ...
    void HAKCFunctionEnforcement::CreateAllTransfers() {
      SmallVector<ManagedHAKCPointerP> SortedPointers;
      for (auto &P : PointerManager.ManagedPointers()) { SortedPointers.push_back(P); }
      ...
      while (PointersUpdated) {
        ...
        for (auto &ManagedPtr : SortedPointers) {
          ...
          if (CurrentAuthUserCount != ManagedPtr->GetAuthenticatedUserCount() || CurrentProtUserCount != ManagedPtr->GetProtectedUserCount()) {
            GetLogger(Verbose, !DebugActive) << *ManagedPtr << " changed user count\n";
      ...
      while (PointersUpdated) {
        ...
        for (auto &ManagedPtr : SortedPointers) {
          const auto OrigBaseIsAuthenticated = ManagedPtr->BaseIsAuthenticatedPointer();
          ...
          if (OrigBaseIsAuthenticated != BaseAuthenticatedResult) {
            GetLogger(Verbose, !DebugActive) << *ManagedPtr << " changed base authentication flag from "
                << std::to_string(OrigBaseIsAuthenticated) << " to " << std::to_string(BaseAuthenticatedResult) << "\n";
            PointersUpdated = true;
          }
        }
      }
    ...
    Value *HAKCFunctionEnforcement::CreateSafePointerAtLocation(Value *Pointer,
    ...
    Value *HAKCFunctionEnforcement::CreateAuthenticationAtLocation(Value *Pointer, Instruction *InsertLocation) {
      if (auto *Managed = PointerManager.FindAuthenticatedValue(Pointer)) {
        return Managed;
      }
     ...
     */
    Instruction *HAKCFunctionEnforcement::FindUseInsertionPoint(Value *V, const std::set<Instruction *> &Users) const {
      if (const auto phi = dyn_cast<PHINode>(V)) {
        return phi->getParent()->getFirstNonPHIOrDbgOrLifetime();
      }

      GetLogger(Verbose, !DebugActive) << "Finding insertion point for " << V << "\n";
      ...
      GetLogger(Verbose, !DebugActive) << "findDominatorUseBlock found " << *DominatorBlock << "\n";

      for (Instruction &I : *DominatorBlock) {
        if (&I == V) { return I.getNextNonDebugInstruction(); }
        if (!isa<PHINode>(&I) && Users.contains(&I)) {
          GetLogger(Verbose, !DebugActive) << "In FindUseInsertionPoint Instruction " << I << "\n";
          return &I;
        }
      }

      GetLogger(Verbose, !DebugActive) << "In FindUseInsertionPoint got DominatorBlock " << DominatorBlock->getTerminator() << "\n";
      return DominatorBlock->getTerminator();
    }

    bool HAKCFunctionEnforcement::FunctionsAreInSameCompartment(Function *F, Function *G) const {
     ...
     */
    Value *HAKCFunctionEnforcement::AddDataAuthCheckAtLocation(Value *SignedPtr, Instruction *location) {
      const auto HAKCPointer = PointerManager.GetManagedPointer(SignedPtr);
      if (!HAKCPointer) {
        GetLogger(Fatal, !DebugActive) << "Could not find Managed Pointer for " << SignedPtr << "\n";
        throw std::exception();
      }
    ...
    Value *HAKCFunctionEnforcement::AddCodeAuthCheckAtLocation(Value *SignedPtr, Instruction *Location) {
      const auto HAKCPointer = PointerManager.GetManagedPointer(SignedPtr);
      if (!HAKCPointer) {
        GetLogger(Fatal, !DebugActive) << "Could not find Managed Pointer for " << SignedPtr << "\n";
        throw std::exception();
    ...
    Value *HAKCFunctionEnforcement::AddSafePointerCreationAtLocation(Value *SignedPtr, Instruction *Location) {
      const auto HAKCPointer = PointerManager.GetManagedPointer(SignedPtr);
      if (!HAKCPointer) {
        GetLogger(Fatal, !DebugActive) << "Could not find Managed Pointer for " << SignedPtr << "\n";
        throw std::exception();
      ...
      GetLogger(Debug, !DebugActive) << "Created Safe Pointer\n\t" << *SafePtr << "\nFor Signed Pointer\n\t"
          << *SignedPtr << "\nat\n" << *Location << "\n";
      return SafePtr;
    }
    ...
    void HAKCFunctionEnforcement::AddInstrumentation() {
      ...
      GetLogger(Verbose, !DebugActive) << "Running AddInstrumentation for " << CurrentFunction->getName() << "\n";
      ...
      if (modifiedFunction()) {
        ...
        GetLogger(Verbose, !DebugActive) << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
        GetLogger(Verbose, !DebugActive) << "----- UpdateHAKCFunctionParameters ------\n";
        UpdateHAKCFunctionParameters();
        GetLogger(Verbose, !DebugActive) << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
        GetLogger(Verbose, !DebugActive) << "---- createAllAuthenticatedPointers ----\n";
        createAllAuthenticatedPointers();
        GetLogger(Verbose, !DebugActive) << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
        GetLogger(Verbose, !DebugActive) << "----- transformPointerDereferences ------\n";
        transformPointerDereferences();
        GetLogger(Verbose, !DebugActive) << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
        GetLogger(Verbose, !DebugActive) << "- ReplaceDirectFunctionUsesWithTransfers -\n";
        ReplaceDirectFunctionUsesWithTransfers();
        GetLogger(Verbose, !DebugActive) << "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
        GetLogger(Verbose, !DebugActive) << "------ CheckForValidCompartmentTransitionAndUpdateIntraCompartmentCalls -----\n";
        ...
        GetLogger(Verbose, !DebugActive) << "Function " << CurrentFunction->getName() << " unmodified\n";
      }
      GetLogger(Verbose, !DebugActive) << "Finished running AddInstrumentation for " << CurrentFunction->getName() << "\n";
    }
    ...
    void HAKCFunctionEnforcement::WriteBuggyFunctionToFile() {
      ...
      GetModuleAnalysis().GetCommonAnalysis().GetModuleFullPath(GetModuleAnalysis().GetModule(), ModulePath);
      sys::path::append(Path, GetModuleAnalysis().GetSystemInformation().GetBuildPath());
    ...
    Instruction *HAKCFunctionEnforcement::addCompartmentTransferCall(
      ...
      if (!Operand->getType()->isPointerTy() && !isa<PtrToIntInst>(Operand) &&
          !Operand->getType()->isIntegerTy(HAKCCompartment::CompartmentIDBitCount)) {
        getLogger(Fatal)
      ...
      GetLogger(Verbose, !DebugActive) << "isData: " << std::to_string(IsData) << " for " << Operand << "\n";

      Instruction *TransferCall = !Size
        ? TransferCall = GetTransformer().CreateCompartmentTransfer(*HAKCPointer, I, CurrentFunction, IsData)
        : TransferCall = GetTransformer().CreateSizedCompartmentTransfer(*HAKCPointer, I, CurrentFunction, IsData, Size);
    ...
    Instruction *HAKCFunctionEnforcement::CreateMissingTransfer(Instruction *PointerNeedingTransfer) {
      ...
      if (auto *Call = dyn_cast<CallInst>(PointerNeedingTransfer)) {
        if (ModuleAnalysis.GetCommonAnalysis().IsAllocationFunction(Call->getCalledFunction())) {
          const auto AllocationDef =
            ModuleAnalysis.GetCommonAnalysis().GetAllocationDefinition(Call->getCalledFunction());
    ...
    Instruction *HAKCFunctionEnforcement::SignGlobalPointerWithColor(GlobalValue *GlobalVar) {
        std::set<Instruction *> UserInstructions;
        for (auto *U: GlobalVar->users()) {
            if (auto *I = dyn_cast<Instruction>(U)) {
                if (I->getFunction() == CurrentFunction) { UserInstructions.insert(I); }
            }
        }
        ...
        return GetTransformer().CreateSignWithDivision(*HAKCPointer, InsertionPoint, CurrentFunction, !isa<Function>(GlobalVar));
    }

    void HAKCFunctionEnforcement::ReplaceInstructionOperand(Instruction *I, const unsigned ArgNo, Value *OldValue, Value *NewValue) {
    ...
    void HAKCFunctionEnforcement::CheckAndReplaceArgument(Value *V, Instruction *I, const unsigned int ArgNo) {
        if (auto *Func = dyn_cast<Function>(V)) {
            const auto name = ModuleAnalysis.GetCommonAnalysis().GetOutsideTransferName(Func);
            const auto transfer = ModuleAnalysis.GetFunctionByName(name, Func->getFunctionType());
            GetLogger(Verbose, !DebugActive) << "Changing operand " << std::to_string(ArgNo) << " to " << name << " for\n\t" << *I << "\n";
    ...
    void HAKCFunctionEnforcement::CreateBaseAuthenticatedPointer(const ManagedHAKCPointerP & ManagedPtr) {
      GetLogger(Verbose, !DebugActive) << __FUNCTION__ << " called for " << *ManagedPtr << "\n";

      if (ManagedPtr->PointerSetsShouldBeEqual()) {
        SetPointerSetsToBeEqual(ManagedPtr);
        if (ManagedPtr->GetPurposefullyIgnored()) {
          GetLogger(Verbose, !DebugActive) << ManagedPtr << " is purposefully ignored\n";
        }
        return;
      }

      if (ManagedPtr->GetAuthenticatedUserCount() == 0) {
        GetLogger(Verbose, !DebugActive) << "No authenticated pointer uses of " << *ManagedPtr << ", so authenticated pointer creation is not needed\n";
        return;
      }
      // in hakc_foo instance, incorrectly entering if statement
      if (ManagedPtr->BaseIsAuthenticatedPointer()) {
        GetLogger(Verbose, !DebugActive) << "The Base Definition is authenticated, so setting uses to be authenticated\n";

        ManagedPtr->SetAuthenticatedPointer(ManagedPtr->GetBaseDefinition());
        SmallVector<ManagedHAKCPointerUseP> SortedUses(ManagedPtr->GetAuthenticatedUses().begin(), ManagedPtr->GetAuthenticatedUses().end());
        SortedUses.append(ManagedPtr->GetCloneUses().begin(), ManagedPtr->GetCloneUses().end());
      ...
      if (ManagedPtr->GetAuthenticatedPointer()) {
        GetLogger(Verbose, !DebugActive) << "AuthenticatedPointer already created\n";
        return;
      }
      ...
      if (Users.empty()) {
        GetLogger(Verbose, !DebugActive) << "User count of " << *ManagedPtr << " is 0. No Authenticated Pointer needed\n";
        return;
      }

      GetLogger(Verbose, !DebugActive) << "Creating Base Authenticated Pointer of " << *ManagedPtr << " with Users\n";
      for (auto &User : Users) {
      ...
      auto *AuthenticationInsertPoint = FindUseInsertionPoint(ManagedPtr->GetBaseDefinition(), UserI);
      if (BaseDefinitionShouldBeTransferred(ManagedPtr) && isa_and_nonnull<Instruction>(ManagedPtr->GetProtectedPointer())) {
        CreateSafePointerOfTransfer = true;
        AuthenticationInsertPoint = dyn_cast<Instruction>(ManagedPtr->GetProtectedPointer())->getNextNonDebugInstruction();
      }

      if (ManagedPtr->GetType() && ManagedPtr->GetType()->IsIgnoredType()) {
        if (auto *I = CreateSafePointerAtLocation(PointerToAuthenticate, AuthenticationInsertPoint)) {
          ManagedPtr->SetAuthenticatedPointer(I);
        }
      } else {
        Value *I = nullptr;
        if (!isCompartmentalizedFunction() || CreateSafePointerOfTransfer) {
          I = CreateSafePointerAtLocation(PointerToAuthenticate, AuthenticationInsertPoint);
        }
        // TODO: look here next
        if (!I) {
          I = CreateAuthenticationAtLocation(PointerToAuthenticate, AuthenticationInsertPoint);
        }
        if (I) {
          ManagedPtr->SetAuthenticatedPointer(I);
        }
      ...
      if (!ManagedPtr->GetAuthenticatedPointer()) {
        getLogger(Fatal) << "Failed to create authenticated pointer for " << *ManagedPtr << " in Function\n"
            << CurrentFunction << "\n";
        throw std::exception();
      }
    }
    ...
    void HAKCFunctionEnforcement::SetPointerSetsToBeEqual(const ManagedHAKCPointerP & ManagedPtr) {
      GetLogger(Verbose, !DebugActive) << ManagedPtr << " setting pointer sets to be equal\n";

      SmallVector<ManagedHAKCPointerUseP> SortedUses(ManagedPtr->GetAuthenticatedUses().begin(), ManagedPtr->GetAuthenticatedUses().end());
      SortedUses.append(ManagedPtr->GetCloneUses().begin(), ManagedPtr->GetCloneUses().end());
      SortedUses.append(ManagedPtr->GetProtectedUses().begin(), ManagedPtr->GetProtectedUses().end());
      ManagedHAKCPointerUse::SortUses(SortedUses);
    ...
    void HAKCFunctionEnforcement::MaybeCreateProtectedPointer(const ManagedHAKCPointerP & ManagedPtr) {
      GetLogger(Verbose, !DebugActive) << __FUNCTION__ << " called for " << ManagedPtr << "\n";
      ...
      if (ManagedPtr->PointerSetsShouldBeEqual()) {
        GetLogger(Verbose, !DebugActive) << *ManagedPtr << " pointer sets will be made equal\n";
        return;
      }
      ...
      if (!ManagedPtr->BaseIsAuthenticatedPointer() && !BaseShouldBeTransferred && !ManagedPtr->GetManuallyTransferred()) {
        ProtectedValue = ManagedPtr->GetBaseDefinition();
      } else if (
        ManagedPtr->GetManuallyTransferred()) {
        GetLogger(Verbose, !DebugActive) << "Transfer not needed for " << *ManagedPtr << " because ProtectedPointer is already set to be " << *ManagedPtr->GetProtectedPointer() << "\n";
        ProtectedValue = ManagedPtr->GetProtectedPointer();
      } else if (BaseShouldBeTransferred) {
        GetLogger(Verbose, !DebugActive) << "Creating Transfer of BaseDefinition of " << *ManagedPtr << "\n";
        ...
        if (auto *GV = dyn_cast<GlobalValue>(ManagedPtr->GetBaseDefinition())) {
          ...
          if (!BaseDefI) {
            GetLogger(Verbose, !DebugActive)
                << "Unexpected BaseDefinition for " << *ManagedPtr << " in function "
      ...
      if (!ProtectedValue && ManagedPtr->GetProtectedUserCount() > 0) {
        getLogger(Fatal)
            << "The protected pointer of " << *ManagedPtr << " is " << ManagedPtr->GetProtectedPointer()
      ...
      if (ProtectedValue) {
        ...
        SmallVector<ManagedHAKCPointerUseP> SortedUses(ManagedPtr->GetProtectedUses().begin(), ManagedPtr->GetProtectedUses().end());
        const bool ReplaceCloneUses = !ManagedPtr->BaseIsAuthenticatedPointer() || ManagedPtr->GetManuallyTransferred();
        if (ReplaceCloneUses) {
          SortedUses.append(ManagedPtr->GetCloneUses().begin(), ManagedPtr->GetCloneUses().end());
        }
        ManagedHAKCPointerUse::SortUses(SortedUses);
    ...
    void HAKCFunctionEnforcement::MaybeCreateBaseCopyPointer(const ManagedHAKCPointerP & ManagedPtr) {
      GetLogger(Verbose, !DebugActive) << __FUNCTION__ << " called for " << *ManagedPtr << "\n";

      /* Note these checks come from CreateBaseAuthenticatedPointer */
      if (ManagedPtr->PointerSetsShouldBeEqual() || ManagedPtr->GetAuthenticatedUserCount() == 0 || ManagedPtr->BaseIsAuthenticatedPointer()) {
        return;
      }

      if (CommonHAKCAnalysis::IsMultiSSAUser(ManagedPtr->GetBaseDefinition()) && ManagedPtr->AllIncomingValuesWillBeAuthenticated()) {
        GetLogger(Verbose, !DebugActive) << "All incoming values will have authenticated versions\n";

        auto *BaseCopy = CloneInstruction(dyn_cast<Instruction>(ManagedPtr->GetBaseDefinition()));
    ...
    void HAKCFunctionEnforcement::CreatePointerReplacements(const ManagedHAKCPointerP & ManagedPtr) {
      const bool CreateAuthenticatedCopies = ManagedPtr->GetAuthenticatedUserCount() > 0;
      const bool CreateProtectedCopies = ManagedPtr->GetProtectedUserCount() > 0;

      if (!CreateAuthenticatedCopies) {
        GetLogger(Verbose, !DebugActive) << "No Authenticated Users of " << *ManagedPtr
      ...
      if (!CreateAuthenticatedCopies && !CreateProtectedCopies) { return; }
      ...
      if (CreateAuthenticatedCopies) {
        ...
        SortedUses.append(ManagedPtr->GetAuthenticatedUses().begin(), ManagedPtr->GetAuthenticatedUses().end());
        ManagedHAKCPointerUse::SortUses(SortedUses);

        for (auto &SortedUse : SortedUses) {
          GetLogger(Verbose, !DebugActive) << "Creating authenticated clone of " << *SortedUse << "\n";
          auto *AuthenticatedUseClone = CreateAuthenticatedValue(ManagedPtr, *SortedUse);
          GetLogger(Verbose, !DebugActive) << "AuthenticatedUseClone: " << *AuthenticatedUseClone << "\n";
        }
      ...
      if (CreateProtectedCopies) {
        ...
        for (auto &SortedUse : SortedUses) {
          GetLogger(Verbose, !DebugActive) << "Creating protected clone of " << *SortedUse << "\n";
          auto *ProtectedUseClone = CreateProtectedValue(ManagedPtr, *SortedUse);
          GetLogger(Verbose, !DebugActive) << "ProtectedUseClone: " << *ProtectedUseClone << "\n";
        }
      ...
      for (auto &SortedUse : SortedUses) {
        if (ManagedPtr->BaseIsAuthenticatedPointer()) {
          if (CreateProtectedCopies) {
            ...
            GetLogger(Verbose, !DebugActive) << "Created Protected clone: " << ProtectedClone << "\n";
          }
          if (CreateAuthenticatedCopies) {
            if (!CreateAuthenticatedValue(ManagedPtr, *SortedUse)) {
              getLogger(Fatal) << "Could not find Authenticated Value for " << *SortedUse << "\n";
              throw std::exception();
            }
          }
        } else {
          if (CreateAuthenticatedCopies) {
            auto *AuthenticatedClone = CreateAuthenticatedValue(ManagedPtr, *SortedUse);
            GetLogger(Verbose, !DebugActive) << "Created Authenticated clone: " << AuthenticatedClone << "\n";
          }
          if (CreateProtectedCopies) {
            if (!CreateProtectedValue(ManagedPtr,*SortedUse)) {
              getLogger(Fatal) << "Could not find Protected Value for " << *SortedUse << "\n";
              throw std::exception();
            }
          }
        }
    ...
    void HAKCFunctionEnforcement::CreatePointerUseClones(const ManagedHAKCPointerP & ManagedPtr) {
      // look here
      ...
      for (auto &UPtr : ManagedPtr->GetAuthenticatedUses()) {
        ...
        if (auto *Replacement = PointerManager.FindAuthenticatedValue(UPtr->get())) {
    ...
    void HAKCFunctionEnforcement::TransformUses(const ManagedHAKCPointerP &ManagedPtr) {
      GetLogger(Verbose, !DebugActive) << __FUNCTION__ << " called for " << *ManagedPtr << "\n";
    ...
    void HAKCFunctionEnforcement::SetUseOperand(const ManagedHAKCPointerP &ManagedPtr,
                                           ...
                                           const bool IsAuthenticatedUse) {
      if (ManagedPtr->GetPurposefullyIgnored()) {
        getLogger(Fatal)
            << "Trying to set operand of purposefully ignored pointer " << *ManagedPtr
            << "\n";
        throw std::exception();
      }

      GetLogger(Verbose, !DebugActive) << "Setting Operand " << std::to_string(PointerUse.getOperandNo()) << " of " << (IsAuthenticatedUse ? "Authenticated" : "Protected");
      GetLogger(Verbose, !DebugActive)
          << " User " << U << " to be " << Replacement << " in function "
          << CurrentFunction->getName() << " for "
          << *ManagedPtr << "\n";

      if (PointerUse.getUser()->getValueID() != U->getValueID()) {
        if (CommonHAKCAnalysis::IsMultiSSAUser(PointerUse.getUser())) {
          GetLogger(Verbose, !DebugActive) << "Not changing operand of MultiSSA User\n";
          return;
        }
        getLogger(Fatal)
    ...
    void HAKCFunctionEnforcement::UpdateUserCounts(const ManagedHAKCPointerP &ManagedPtr) {
      GetLogger(Verbose, !DebugActive)
    ...
    void HAKCFunctionEnforcement::TransformClones(const ManagedHAKCPointerP &ManagedPtr) {
      if (ManagedPtr->GetPurposefullyIgnored()) {
        GetLogger(Verbose, !DebugActive) << "Not transforming clones since " << *ManagedPtr << " is purposefully ignored\n";
        return;
      }

      GetLogger(Verbose, !DebugActive) << "Transforming clones created for " << *ManagedPtr << "\n";
      for (auto &CloneUse : ManagedPtr->GetCloneUses()) {
        GetLogger(Verbose, !DebugActive) << "\t" << *CloneUse << "\n";
      }

      SmallVector<ManagedHAKCPointerUseP> SortedUses(ManagedPtr->GetCloneUses().begin(), ManagedPtr->GetCloneUses().end());
      ManagedHAKCPointerUse::SortUses(SortedUses);
      for (const auto &CloneUse : SortedUses) {
        GetLogger(Verbose, !DebugActive) << "Handling Clone " << *CloneUse->getUser() << "\n";
        if (ManagedPtr->GetAuthenticatedUserCount() > 0) {
          GetLogger(Verbose, !DebugActive) << "Handling Authenticated Use of " << *CloneUse << "\n";
          auto *AuthenticatedVersion = PointerManager.FindAuthenticatedValue(CloneUse->getUser());
          if (!AuthenticatedVersion) {
            GetLogger(Verbose, !DebugActive) << "No authenticated value created for " << *CloneUse << "\n";
          } else {
            ...
            SetUseOperand(ManagedPtr, AuthenticatedUser, Replacement, *CloneUse, true);
          }
        }
        if (ManagedPtr->GetProtectedUserCount() > 0) {
          GetLogger(Verbose, !DebugActive) << "Handling Protected Use of " << *CloneUse << "\n";
          auto *ProtectedVersion = PointerManager.FindProtectedValue(CloneUse->getUser());
          if (!ProtectedVersion) {
            GetLogger(Verbose, !DebugActive) << "No protected value created for " << *CloneUse << "\n";
            continue;
          }
          auto *ProtectedUser = dyn_cast<User>(ProtectedVersion);
          auto *Replacement = PointerManager.FindProtectedValue(*CloneUse);
          if (!Replacement) {
            if (PointerManager.GetManagedPointer(CloneUse->get()) == nullptr || ManagedPtr->UseIsManagedAndHasUsers(*CloneUse, false)) {
              getLogger(Fatal) << "Unable to find Protected replacement of " << *CloneUse << "\n";
              PointerManager.PrintProtectedValues();
              throw std::exception();
            }
            GetLogger(Verbose, !DebugActive) << *CloneUse << " does not need protected operand replaced\n";
            continue;
          }
          if (!ProtectedUser) {
    ...
    void HAKCFunctionEnforcement::TransformUseSet(const ManagedHAKCPointerP &ManagedPtr, SmallVectorImpl<ManagedHAKCPointerUseP> &UseSet) {
      if (ManagedPtr->GetPurposefullyIgnored()) {
        GetLogger(Verbose, !DebugActive) << "Not transforming uses since " << *ManagedPtr << " is purposefully ignored\n";
        return;
      }

      const bool UseAuthenticatedValue = (&UseSet == &ManagedPtr->GetAuthenticatedUses());
      const StringRef ReplacementSource = UseAuthenticatedValue ? "Authenticated" : "Protected";

      ManagedHAKCPointerUse::SortUses(UseSet);

      GetLogger(Verbose, !DebugActive) << "Replacing the following operands with " << ReplacementSource << " values\n";
      for (auto &Use : UseSet) {
        GetLogger(Verbose, !DebugActive) << "\t" << *Use << "\n";
      }
      ...
      for (const auto &SortedUse : UseSet) {
        ...
        if (UseAuthenticatedValue) {
          ...
          if (!CommonHAKCAnalysis::IsMultiSSAUser(SortedUse->getUser()) && ManagedPtr->ValueIsManagedAndHasUsers(SortedUse->getUser(), true)) {
            ReplacementUser = SortedUse->getUser();
          } else {
            ReplacementUser = PointerManager.FindAuthenticatedValue(SortedUse->getUser());
          }
        } else {
          Replacement = PointerManager.FindProtectedValue(*SortedUse);
          if (!CommonHAKCAnalysis::IsMultiSSAUser(SortedUse->getUser()) && ManagedPtr->ValueIsManagedAndHasUsers(SortedUse->getUser(), false)) {
            ReplacementUser = SortedUse->getUser();
          } else {
            ReplacementUser = PointerManager.FindProtectedValue(SortedUse->getUser());
          }
        }
        ...
        SetUseOperand(ManagedPtr, dyn_cast<User>(ReplacementUser), Replacement, *SortedUse, UseAuthenticatedValue);
      }
    }

    Value *HAKCFunctionEnforcement::CreateAuthenticatedValueHelper(ManagedHAKCPointerUse &use) {
      auto *Pointer = use.get();

      if (auto *AuthenticatedCopy = PointerManager.FindAuthenticatedValue(use)) {
        GetLogger(Verbose, !DebugActive) << "Returning Authenticated Copy " << AuthenticatedCopy << " for " << use << "\n";
        return AuthenticatedCopy;
      }

      if (auto *I = dyn_cast<Instruction>(Pointer)) {
        auto *Clone = CloneInstruction(I);
        GetLogger(Verbose, !DebugActive) << "Created Authenticated Copy of " << *I << ": " << Clone << "\n";
        return Clone;
      }
      return nullptr;
    }

    Value *HAKCFunctionEnforcement::CreateAuthenticatedValue(const ManagedHAKCPointerP &ManagedPtr, ManagedHAKCPointerUse &use) {
      if (ManagedPtr->GetPurposefullyIgnored()) {
        return use.get();
      }
      ...
      if (!Authenticated) {
        GetLogger(Verbose, !DebugActive) << "CreateAuthenticatedValueHelper returned null for " << use << "\n";
      } else {
    ...
    Value *HAKCFunctionEnforcement::CreateProtectedValueHelper(ManagedHAKCPointerUse &PointerUse) {
      auto *Pointer = PointerUse.get();

      if (auto *ProtectedValue = PointerManager.FindProtectedValue(PointerUse)) {
        GetLogger(Verbose, !DebugActive) << "Returning Protected Version " << *ProtectedValue << " for " << PointerUse << "\n";
      ...
      if (auto *I = dyn_cast<Instruction>(Pointer)) {
        ...
        GetLogger(Verbose, !DebugActive) << "Created Protected Version of " << *I << ": " << Clone << "\n";
        return Clone;
      }
      return nullptr;
    }

    Value *HAKCFunctionEnforcement::CreateProtectedValue(const ManagedHAKCPointerP &ManagedPtr, ManagedHAKCPointerUse &HAKCUse) {
      if (ManagedPtr->GetPurposefullyIgnored()) {
        return HAKCUse.get();
      }
      ...
      if (!Protected) {
        GetLogger(Verbose, !DebugActive) << "CreateProtectedValue returned null for " << HAKCUse << "\n";
      } else {
    ...
    bool HAKCFunctionEnforcement::BaseDefinitionShouldBeTransferred(const ManagedHAKCPointerP &ManagedPtr) const {
      ...
      if (const auto *Call = dyn_cast<CallInst>(ManagedPtr->GetBaseDefinition())) {
        ...
        return ModuleAnalysis.GetCommonAnalysis().IsAllocation(ManagedPtr->GetBaseDefinition()) || !FunctionsAreInSameCompartment(CurrentFunction, Callee);
      }
      if (ManagedPtr->BaseIsAuthenticatedPointer()) {
