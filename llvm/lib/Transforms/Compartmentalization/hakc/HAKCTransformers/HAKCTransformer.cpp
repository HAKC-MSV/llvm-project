...
void HAKCTransformer::runEnforcement() {
  TransformModule();
  getLogger(Debug) << "Final Module After Transformations:\n"
                   << getModule() << "\n";
}
...
void HAKCTransformer::TransformFunctions() {
  for (auto *F : ModuleAnalysis.AnalysisFunctions) {
    HAKCFunctionEnforcement FunctionEnforcement(F, ModuleAnalysis, (*this),
                                                Client);
  }
}
...
void HAKCTransformer::AddTransferFunctions() {
  ...
  for (auto *Funcp : FuncsNeedingTransfers) {
    ...
    if (getCommonAnalysis().functionIsTransferCandidate(&F, Client)) {
      ...
      getLogger(Verbose) << (TransferAlreadyExisted
                                 ? "Retrieved transfer function "
                                 : "Created transfer function ")
                         << transferFunc->getName();
      getLogger(Verbose) << " in compartment "
    ...
    if (getCommonAnalysis().ValueShouldBeReplacedWithTransfer(&F, Client)) {
      if (!getCommonAnalysis().IsNoTransferFunction(&F)) {
        ...
        for (auto &[key, val] : UsesToReplace) {
          getLogger(Verbose) << "Replacing operand " << val << " of " << key
                             << " with " << transferFunc << "\n";
          if (auto *Const = dyn_cast<Constant>(key)) {
            ...
            if (const auto *ConstArray = dyn_cast<ConstantArray>(key)) {
            ...
            } else if (const auto *ConstStruct =
              ...
              getLogger(Fatal)
                  << "Unhandled ConstantAggregate: " << key << "\n";
              throw std::exception();
            }

            for (auto &InitializerUser : key->uses()) {
              if (auto *GV =
                      dyn_cast<GlobalVariable>(InitializerUser.getUser())) {
...
bool HAKCTransformer::ConstantStructTransferIsNeeded(
  ...
  for (auto &Member : ConstStruct->operands()) {
    ...
    if (auto *GlobalVal = dyn_cast<GlobalValue>(Def)) {
      Result = !CommonHAKCAnalysis::IsNECSymbol(GlobalVal, Client);
...
bool HAKCTransformer::TransferIsNeeded(GlobalVariable *GlobalVar) {
  bool IsKernelSym = CommonHAKCAnalysis::IsNECSymbol(GlobalVar, Client);
...
void HAKCTransformer::CreateDataAuthArguments(
  ...
  const auto *DataAuthFuncTy =
      getCommonAnalysis().GetDataAuthenticationFunctionType();

  Value *HAKCPointerBitCast =
      HAKCPointer.GetBaseDefinition()->getType()->isIntegerTy()
          ? HAKCIRBuilder.CreateIntToPtr(HAKCPointer.GetBaseDefinition(),
                                         DataAuthFuncTy->getParamType(0))
          : HAKCIRBuilder.CreateBitCast(HAKCPointer.GetBaseDefinition(),
                                        DataAuthFuncTy->getParamType(0));
...
void HAKCTransformer::CreateTransferArguments(HAKCPointerBase &HAKCPointer,
  ...
  Value *OperandCast = HAKCIRBuilder.CreateBitOrPointerCast(
      HAKCPointer.GetBaseDefinition(), HAKCIRBuilder.getPtrTy(AddrSpace));
...
void HAKCTransformer::ValidateHAKCPointer(const HAKCPointerBase &HAKCPointer) {
  ...
  if (!HAKCPointer.GetType()->GetPointeeType()) {
    getLogger(Fatal) << "HAKCPointer " << HAKCPointer << " Type "
                     << *HAKCPointer.GetType() << " has no PointeeType\n";
    throw std::exception();
  }
}
...
Value *HAKCTransformer::CreateDataAuthentication(HAKCPointerBase &HAKCPointer,
  ...
  if (isa<PHINode>(I)) {
    getLogger(Fatal) << "Trying to insert data auth check at " << *I << " for "
                     << HAKCPointer << "\n"
                     << *I->getFunction();
    throw std::exception();
  }
...
Value *HAKCTransformer::CreateReturnCast(HAKCPointerBase &HAKCPointer,
                                         Value *V) {
  if (!V) {
...
Value *HAKCTransformer::CreatePointerCast(HAKCPointerBase &HAKCPointer,
                                          PointerType *PointerTy) {
  if (!PointerTy) {
...
Instruction *HAKCTransformer::CreateSizedCompartmentTransfer(
  ...
  Instruction *Transfer =
      HAKCPointerHasCustomTransfer(HAKCPointer)
...
HAKCTransformer::CreateDefaultTransfer(HAKCPointerBase &HAKCPointer,
  ...
  auto CompartmentTransfer = getSystemInfo().CompartmentTransfer(
      ModuleAnalysis.GetCommonAnalysis().IsPerCPUPointer(
          HAKCPointer.GetBaseDefinition()));
...
Instruction *HAKCTransformer::CastCallToType(CallInst *Call,
                                             Value *ValueToTypeMatch) {
  Value *ResultCast =
      isa<PtrToIntInst>(ValueToTypeMatch) ||
