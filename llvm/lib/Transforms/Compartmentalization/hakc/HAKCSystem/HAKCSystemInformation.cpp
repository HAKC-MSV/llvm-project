...
namespace llvm::hakc {
    ...
    HAKCSystemInformation::HAKCSystemInformation(CommonHAKCAnalysis &CommonAnalysis)
        : CommonAnalysis(CommonAnalysis), TypeIdentifier(CommonAnalysis),
          ...
          AllocationFunctionList(), CustomTransferList(), PreTransferActionList(), PostTargetActionList() {
    }
    ...
    hakc::custom_transfer_def_t HAKCSystemInformation::CreateCustomTransferFunction(
        HAKCYAMLCustomTransferType &YAMLCustomTransfer, HAKCTypeP HAKCTy, HAKCTypeIdentifier &TypeIdentifier) {
        auto *TransferFunc = YAMLCustomTransfer.GetFunction(TypeIdentifier);
        if (!TransferFunc) {
            CommonHAKCAnalysis::getWriter(true) << "Could not find function " << YAMLCustomTransfer.Name << "\n";
            throw std::exception();
        }
        SmallVector<HAKCFunctionArgumentDefinition> Args;
    ...
    void HAKCSystemInformation::PopulateHAKCFunctionArgs(SmallVectorImpl<HAKCFunctionArgumentDefinition> &Args,
        ...
        for (auto &YAMLArg: YAMLFunctionDef.Arguments) {
            ...
            if (!ArgTy) {
                CommonHAKCAnalysis::getWriter(true) << "Could not determine type for argument " << YAMLArg.Idx <<
                        " in definition for " << YAMLFunctionDef.Name << "\n";
                throw std::exception();
            }
    ...
    void HAKCSystemInformation::operator<<(HAKCYamlConfig &YamlConfig) {
        ...
        DefaultCompartmentTransfer = CreateHAKCFunction(YamlConfig.DefaultCompartmentTransfer, TypeIdentifier);
        if (YamlConfig.PerCPUCompartmentTransfer.IsValid()) {
            PerCPUCompartmentTransfer = CreateHAKCFunction(YamlConfig.PerCPUCompartmentTransfer, TypeIdentifier);
        } else {
        ...
        for (auto &CustomTransferDefinition: YamlConfig.CustomTransferFunctions) {
            for (auto &HAKCTy: Types) {
                if (CustomTransferDefinition.TransferObjectTypeName == HAKCTy) {
                    auto CustomTransfer =
                            CreateCustomTransferFunction(CustomTransferDefinition, HAKCTy, TypeIdentifier);
