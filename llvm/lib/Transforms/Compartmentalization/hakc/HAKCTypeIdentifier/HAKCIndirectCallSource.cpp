...
HAKCIndirectCallSource::HAKCIndirectCallSource(
    ...
    const std::shared_ptr<HAKCTypeInfo> &HAKCType, const bool debug)
...
void HAKCIndirectCallSourceLink::SplitTypeYaml(
...
HAKCIndirectCallSourceLink::HAKCIndirectCallSourceLink(
    Argument *Arg, const std::shared_ptr<HAKCTypeInfo> &HAKCType, bool Debug)
    : HAKCInfo(HAKCType->GetCommonHAKCAnalysis(), "Argument Indirect Call Link",
               Debug),
      LinkYamlTokens() {
  InputYamlHeader();
...
HAKCIndirectCallSourceLink::HAKCIndirectCallSourceLink(
    const std::shared_ptr<HAKCGlobalInfo> &GlobalInfo, bool Debug)
    : HAKCInfo(GlobalInfo->GetCommonHAKCAnalysis(), "Global Indirect Call Link",
               Debug),
      LinkYamlTokens() {
  InputHAKCSymbol(GlobalInfo);
}

void HAKCIndirectCallSourceLink::InputHAKCSymbol(
    const std::shared_ptr<HAKCSymbolInfo> &HAKCSymbol) {
  InputYamlHeader();
...
HAKCIndirectCallSourceLink::HAKCIndirectCallSourceLink(
    const std::shared_ptr<HAKCSymbolInfo> &HAKCSymbol, bool Debug)
    : HAKCInfo(HAKCSymbol->GetCommonHAKCAnalysis(), "Global Indirect Call Link",
               Debug),
      LinkYamlTokens() {
...
HAKCIndirectCallSourceLink::HAKCIndirectCallSourceLink(
    const std::shared_ptr<HAKCGlobalInfo> &GlobalInfo, int OffsetInBits,
    bool Debug)
    : HAKCInfo(GlobalInfo->GetCommonHAKCAnalysis(),
               "Global Member Indirect Call Link", Debug),
      LinkYamlTokens() {
  InputYamlHeader();
...
HAKCIndirectCallSourceLink::HAKCIndirectCallSourceLink(
    const std::shared_ptr<HAKCTypeInfo> &HAKCType, int OffsetInBits, bool Debug)
    : HAKCInfo(HAKCType->GetCommonHAKCAnalysis(), "Type member dereference",
               Debug),
      LinkYamlTokens() {
  InputYamlHeader();
...
void HAKCIndirectCallSourceLink::InputType(
    const std::shared_ptr<HAKCTypeInfo> &HAKCType) {
  std::string Yaml;
  raw_string_ostream sstream(Yaml);
...
void HAKCIndirectCallSourceLink::InputGlobalObject(GlobalObject *GlobalObj) {
  ...
  sstream.indent(EntrySpaces())
      << "GlobalName: " << "\"" << GlobalObj->getName() << "\"";
  LinkYamlTokens.push_back(Yaml);
}
...
void HAKCIndirectCallSourceLink::InputArgument(Argument *Arg) {
  ...
  Yaml = "";
  sstream.indent(EntrySpaces())
      << "Function: \"" << Arg->getParent()->getName() << "\"";
  LinkYamlTokens.push_back(Yaml);
}
